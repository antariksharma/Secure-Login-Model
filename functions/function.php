<?php
   ...
   //Item Name and Decription Validation
  
   function addItem($Name,$Decription, $SContact, $SEmail)
   {
    global $con;
    $item_name = mysqli_real_escape_string($con,$Name);
    $item_description = mysqli_real_escape_string($con,$Decription);
    $Contact = $SContact;
    $Email = $SEmail;

    $sql = "UPDATE users SET item_name='$item_name', item_description='$item_description', contact_method='$Contact' where Email=$Email";
   ...
   function user_registration($First,$Last,$User,$Email,$Password)
   {
        ...
        $Pass = md5($Password);
         $Validation_Code = md5($UserName.microtime());
          ...
          $sql = "INSERT INTO users (FirstName,LastName,UserName,Email,Password,Validation_Code,Active,file_name,item_name,item_description,contact_method) VALUES ('$FirstName','$LastName','$UserName','$Email','$Pass','$Validation_Code','0','','','','')";
        ...
        $Msg = " Please Active Your Account to Click the Link Below : https://lovejoyantiquesitemshop.000webhostapp.com/activate.php?Email=$Email&Code=$Validation_Code";
       ...
       if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btn-login']))
       {
           ...
           {   
               ...
               {   
                   ...
                   echo '<div class="alert alert-success"> Please Check Your Email to verify this you :) </div>';
   ...
   function login($Email,$Password,$Remember)
   {
       ...
       if($row=mysqli_fetch_assoc($result))
       {
           ...
           if(md5($Password)==$db)
           {
            ...
            if(isset($_SESSION['token']) && $_POST['token_login'] == $_SESSION['token'])
            {
             oneTimePass($Email);
            }
               ...
               $_SESSION['Email']=$Email;
                ...
                //redirect to Admin Page if true
                return true;
           }
           else
           {
   ...
   function LoveJoyOwner()
   {
       $admin= "lovejoym404@gmail.com";
       
       if(logged_in() && $_SESSION['Email'] == $admin)
       {
           return true;
       }
   ...
   function oneTimePass ($Email)
   {
                ...
                if(Email_Exist($Email))
                {
                    ...
                    setcookie('otp_temp',$Code,time() + 900);
                    $query = "update users set Validation_Code='$Code' where Email='$Email'";
                    mysqli_query($con,$query);

                    $Subject = " Your One Time Password for logging in ";
                    $Message = " Your One Time Pass is {$Code} please click here 
                    https://lovejoyantiquesitemshop.000webhostapp.com/otp.php?Email='$Email'&Code='$Code'";
                    $header = "no-reply@lovejoy.com";
  ...
  /*************OneTimePass Validation***************** */
  function OTP_Validation()
  {
      global $con;
      if(isset($_COOKIE['otp_temp']))
      {
            if(isset($_GET['Email']) && isset($_GET['Code']))
            {
                ...
                {
                    $Email = $_GET['Email'];
                    $Code = mysqli_real_escape_string($con,$_POST['otp_code']);
                    //echo $Email;
                    //echo $Code;

                    $sql = " SELECT * FROM users WHERE Validation_Code = '$Code' AND Email = $Email ";
                    $result = mysqli_query($con,$sql);

                    if(mysqli_fetch_assoc($result))
                    {
                        setcookie('otp_temp',$Code,time() + 600);
      ...
      if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['btn_recover']))
      {
          if(isset($_SESSION['token']) && $_POST['token_input'] == $_SESSION['token'])
          {
                ...
                if(Email_Exist($Email))
                {
                    ...
                    $Message = " Your Reset Validation Code is {$code} please click here https://lovejoyantiquesitemshop.000webhostapp.com/code.php?Email='$Email'&Code='$code'";
                    $header = "no-reply@onlineittuts.com";
  ...
  function validation_code()
  {
      ...
      if(isset($_COOKIE['temp_code']))
      {
            if(isset($_GET['Email']) && isset($_GET['Code']))
            {
                ...
                {
                    $Email = $_GET['Email'];
                    $Code = mysqli_real_escape_string($con,$_POST['token_code']);
                    //echo $Email;
                    //echo $Code;
          ...
          if(isset($_GET['Email']) && isset($_GET['Code']))
          {
               if(isset($_SESSION['token']) && isset($_POST['pass_token']))
               {
                   ...
                   {    
                        ...
                        if(!$number) {
                            $Errors [] = "Password should contain atleast some numbers";
                            $change = 0;
                        }
                       ...
                       {
                        $Errors []= " Your Password Not Matched ";
                        $change = 0;
                       }
                       ...
                       if(!empty($Errors))
                        ...
                        if ($change == 0){
                            $Errors [] = "Sorry there was a problem with changing your password. Please try again";
                        }
